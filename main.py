...
import pandas as pd
import pytz
import yfinance as yf  # If you get an import error, ensure yfinance is installed
...
# Initialize Gemini model - without the system_instruction parameter
...
def chatbot():
    ...
    ticker = data.get("ticker", "").strip().upper() if data else ""
    ...
    try:
        ...
        if not ticker:
            ...
            ticker = next(
                (t for t in potential_tickers if t in SUPPORTED_TICKERS), None
            )
        ...
        # Combine system prompt with user query for models that don't support system prompts directly
        prompt = f"{sys_prompt}\n\nUser query: {user_query}\n\nPlease provide a concise answer (2-3 paragraphs maximum)."

        response = gemini.generate_content(prompt)
        return jsonify(
            {"status": "success", "response": response.text, "is_stock_response": False}
        )
...
def generate_stock_insights(ticker, stock_info):
    ...
    if profit_margins:
        ...
        insights.append(
            f"Financial health appears {financial_health} with {profit_margins*100:.1f}% profit margins."
        )
    ...
    if fifty_two_week_change:
        ...
        insights.append(
            f"Has {performance} the market over past year with {fifty_two_week_change*100:.1f}% change."
        )
    ...
    if rec:
        insights.append(
            f"Analyst consensus is currently '{rec}' with target price of ${stock_info.get('targetMeanPrice', 'N/A')}."
        )
    ...
    if pe:
        valuation = (
            "overvalued" if pe > 25 else "undervalued" if pe < 15 else "fairly valued"
        )
        insights.append(
            f"Valuation appears {valuation} with P/E ratio of {pe:.1f} compared to industry average."
        )
    ...
    if inst_own:
        insights.append(
            f"Institutional ownership stands at {inst_own*100:.1f}%, indicating {'strong' if inst_own > 0.7 else 'moderate' if inst_own > 0.4 else 'limited'} investor confidence."
        )
    ...
    if rev_growth:
        ...
        insights.append(
            f"Revenue growth is {growth} at {rev_growth*100:.1f}% year-over-year."
        )
    ...
    if stock_info.get("dividendRate", 0) > 0:
        ...
        insights.append(
            "Does not currently pay dividends, reinvesting earnings into growth."
        )
    ...
    if mkt_cap:
        ...
        insights.append(
            f"Positioned as a {position} in its industry with ${mkt_cap/1e9:.1f}B market cap."
        )
    ...
    if beta:
        ...
        insights.append(
            f"Systematic risk appears {risk} with beta of {beta:.2f} compared to the market."
        )
        ...
        if period == "1d":
            ...
            if not data.empty:
                if not data.index.tz:
                    ...
                    market_data = data.tz_convert("America/New_York").between_time(
                        "09:30", "16:00"
                    )
                ...
                if len(market_data) < 2:
                    ...
                    data = stock.history(
                        start=prev_trading_day,
                        end=prev_trading_day + timedelta(days=1),
                        interval="1m",
                        prepost=True,
                    )
                    if not data.empty:
            ...
            graph_data = [
                {
                    "time": idx.tz_convert("America/New_York").strftime(
                        "%Y-%m-%dT%H:%M:%S%z"
                    ),
                    "price": float(row["Close"]),
                    "open": float(row["Open"]),
...
def assess_risk(ticker):
    """Calculate risk assessment for a given ticker"""
    stock = yf.Ticker(ticker)
    ...
    if len(closing_prices) < 10:
        raise ValueError("Not enough data to assess risk")

    volatility = np.std(closing_prices)
    avg_price = np.mean(closing_prices)
    risk_level = (
        "low"
        if volatility < 0.02 * avg_price
        else "medium"
        if volatility < 0.05 * avg_price
        else "high"
    )
...
def favicon():
    return send_from_directory(
        "frontend/assets", "favicon.ico", mimetype="image/vnd.microsoft.icon"
    )
...
if __name__ == "__main__":
    ...
    # Run the app binding to all interfaces (0.0.0.0) for proper cloud deployment
